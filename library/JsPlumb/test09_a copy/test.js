...
fetch('demo.json')
   ...
   .then((data) => {
      ...
      // ⭐ 노드 생성 → left/top 주지 않음 (ELK 후 적용 예정)
      ...
      const elkGraph = {
         ...
         layoutOptions: {
            ...
            // optional: hierarchyHandling 추가하면 경고 방지 가능
      ...
      elk.layout(elkGraph).then((graph) => {
         ...
         graph.children.forEach((node) => {
            ...
            if (div) {
               ...
               console.warn(`⚠️ DOM not found for node.id=${node.id}`);
            }
         });
         ...
         graph.edges.forEach((edge) => {
            ...
            function aStar(grid, start, end, cols, rows) {
               ...
               while (openSet.length > 0) {
                  openSet.sort(
                     (a, b) =>
                        (fScore[key(a)] || Infinity) -
                        (fScore[key(b)] || Infinity)
                  );
      ...
      // ⭐ A* 알고리즘 (그대로 사용 — 생략 가능하면 말씀 주세요!)

      // ⭐ Path → Waypoints (그대로 사용 — 생략 가능하면 말씀 주세요!)
   });
